...
import { requestOtp as sendOtp, verifyOtp as checkOtp } from "../services/otpService.js";
import { asyncHandler } from "../utils/asyncHandler.js";
import { requestOtpBodySchema, verifyOtpBodySchema } from "../utils/validation.js";
...
export const requestOtpHandler = asyncHandler(async (req: Request, res: Response) => {
  ...
  const existingUser = await UserModel.findOne({ phoneNumber: normalizedPhoneNumber });
  if (existingUser?.status === "BLOCKED") {
    throw new ApiError(403, "This number is blocked from voting.");
  }

  const lastOtp = await OtpModel.findOne({ phoneNumber: normalizedPhoneNumber }).sort({ createdAt: -1 });
